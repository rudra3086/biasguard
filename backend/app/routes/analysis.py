...
async def explain_bias(request: ExplainRequest):
    """
    Generate AI explanation for bias analysis results using Gemini.
    
    Parameters:
        - bias_analysis: Complete bias analysis JSON output from /analyze
    
    Returns:
        - status: 'success' or 'error'
        - model: Which explanation model was used (gemini-1.5-flash or fallback)
        - analysis: Human-readable explanation of bias findings
        - fairness_score: Fairness metric (0-1)
        - high_bias_count: Number of high severity bias features
        - medium_bias_count: Number of medium severity bias features
    """
    try:
