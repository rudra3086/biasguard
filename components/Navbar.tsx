...
export default function Navbar({ onUploadClick }: NavbarProps) {
  ...
  return (
    <nav
      className="sticky top-0 z-50 px-4 sm:px-6 py-3 nav-frost"
    >
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-1">
          {['Dashboard', 'Models', 'Reports', 'Settings'].map((item, i) => (
            <button
              ...
              className="nav-pill px-4 py-2 text-sm rounded-xl transition-all duration-200 hover:text-white"
              style={{
                color: i === 0 ? 'var(--text-primary)' : 'var(--text-muted)',
                background: i === 0 ? 'rgba(255,255,255,0.04)' : 'transparent',
                borderColor: i === 0 ? 'rgba(96,165,250,0.25)' : 'transparent',
              }}
            >
              {item}
            </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="btn-primary flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 hover:opacity-90"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {isMenuOpen && (
        <div className="md:hidden mt-3 pb-2 space-y-1" style={{ borderTop: '1px solid var(--border)' }}>
          {['Dashboard', 'Models', 'Reports', 'Settings'].map((item, i) => (
            <button
              key={item}
              className="w-full nav-pill text-left px-4 py-2.5 text-sm rounded-xl transition-all duration-200"
              style={{
                color: i === 0 ? 'var(--text-primary)' : 'var(--text-muted)',
                background: i === 0 ? 'rgba(255,255,255,0.04)' : 'transparent',
                borderColor: i === 0 ? 'rgba(96,165,250,0.25)' : 'transparent',
